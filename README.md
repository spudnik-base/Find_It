...
Browser-based spot-the-match card generator for teachers. Upload your own words or images; FIND IT produces a print-ready card deck where every pair of cards shares exactly one symbol.
...
Word symbols can include markdown-style sub/superscript tokens so chemistry formulas and ions render correctly:

- `H~2~O` renders as H with a small subscript 2 then O.
- `Fe^2+^` renders Fe with a small raised 2+.

Plain labels bypass this path; only labels containing `~` or `^` use the rich renderer.
...
- `js/layout.js`: OBB + SAT circle packing, sub/sup parsing
...
- `js/packs.js`: starter content packs
- `js/app.js`: UI wiring
...
- `findit-settings-v1`: UI preferences (card shape, intro dismissed)
